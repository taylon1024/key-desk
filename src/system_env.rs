...
/// 重名提示里对「系统里已有这个变量」的叫法。macOS / Linux 仍是登录 shell。
pub fn external_source_name() -> &'static str {
    #[cfg(windows)]
    {
...
/// 登录 shell（Windows 上是用户 + 系统环境）里是否已有这个变量。未设置时返回 `Ok(None)`。
pub fn lookup_login_shell_var(key: &str) -> Result<Option<String>, String> {
    #[cfg(windows)]
    {
        let vars = list_windows_user_env()?;
        Ok(vars
            .into_iter()
            .find(|var| var.key.eq_ignore_ascii_case(key))
            .map(|var| var.value))
    }
    #[cfg(not(windows))]
    {
        lookup_unix_login_shell_var(key)
    }
}

#[cfg(not(windows))]
fn lookup_unix_login_shell_var(key: &str) -> Result<Option<String>, String> {
