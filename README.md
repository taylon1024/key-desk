...
- `main` 和发布构建默认在每次启动时请求 Touch ID 或 Mac 账户密码；`dev` 分支默认跳过。验证状态见 [开发进度](docs/DEVELOPMENT_PROGRESS.md)。

## 本地开发

需要 Rust 工具链与 macOS。`main` 默认要求 Touch ID 或 Mac 账户密码：

```bash
cargo run
```

在 `main` 上显式启用开发模式，可跳过启动解锁：

```bash
cargo run --features dev
```

`dev` 分支默认启用 `dev` feature，直接运行 `cargo run` 即可跳过解锁。
